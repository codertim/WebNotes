...
function Notes() {
    ...
    return (
            <div id="notes" className='box-border' style={{alignItems: 'center', display: 'flex', flexDirection: 'column', gap: '2rem'}}>
                ...
                <textarea id="msg-text-area" ref={_textAreaEl} rows={10} cols={50}
                          style={ {backgroundColor: '#72bcd4', overflow: 'auto', caretColor: "yellow"} }
                          defaultValue={initialNotes}
                          className="shadow rounded font-semibold border-2 leading-6 p-1 md:p-4 hover:border-solid hover:border-blue-500"
                          ></textarea>
                <div>
                    <button type="button" id="update-button" style={{margin: '1rem', boxShadow: 'inset 4px 4px 8px rgba(0, 0, 0, .75)'}} className="hover:bg-blue-400 border-slate-200 h-10 px-6 bg-orange rounded-lg text-red w-72" onClick={updateNotes}>Update123</button>
                </div>
            </div>
    );
}
